...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    hidden, cache_hidden_layer = affine_relu_forward(X,self.params['W1'],self.params['b1'])
    ...
    dx2, grads['W1'], grads['b1'] = affine_relu_backward(dx1, cache_hidden_layer)

    grads['W2'] += self.reg*self.params['W2']
    grads['W1'] += self.reg*self.params['W1']


    l1 = 0.5*self.reg*np.sum(self.params['W1']**2)
    l2 = 0.5*self.reg*np.sum(self.params['W2']**2)
    loss += l1
    loss += l2
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(1,layernum+1):
      ...
      if i != layernum and use_batchnorm:
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        ...
        keyb = 'b' + str(i)
        if self.use_batchnorm:
            curr, affine_bn_relu_cache[i] = affine_bn_relu_forward(curr, self.params[keyW], self.params[keyb],
                                                                                   self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
        else:   
            curr, ar_cache[i] = affine_relu_forward(curr, self.params[keyW], self.params[keyb])

        if self.use_dropout:
            curr, dropout_cache[i] = dropout_forward(curr,self.dropout_param)  


    # Last affine layer:
    keyW = 'W' + str(self.num_layers)
    keyb = 'b' + str(self.num_layers)
    scores, affine_cache = affine_forward(curr, self.params[keyW], self.params[keyb])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    #last layer:
    a_dx, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dscores, affine_cache)
    ...
    for i in range(self.num_layers-1,0,-1):
        if self.use_dropout:
            a_dx = dropout_backward(a_dx, dropout_cache[i])

        if self.use_batchnorm:    
          a_dx, a_dw, a_db, grads['gamma'+str(i)], grads['beta'+str(i)] = affine_bn_relu_backward(a_dx, affine_bn_relu_cache[i])    
        else:       
          a_dx, a_dw, a_db = affine_relu_backward(a_dx, ar_cache[i])
        
        keyW = 'W' + str(i)
        keyb = 'b' + str(i)    
        loss += 0.5 * self.reg*(np.sum(self.params[keyW]* self.params[keyW])) 
        grads[keyW] = a_dw + self.reg * self.params[keyW]
        grads[keyb] = a_db
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
