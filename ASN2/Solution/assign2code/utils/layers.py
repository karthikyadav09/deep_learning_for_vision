...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    running_mean = momentum * running_mean + (1 - momentum) * sample_mean
    running_var = momentum * running_var + (1 - momentum) * sample_var
    denom = np.sqrt(sample_var.T + eps)
    out = ((x - sample_mean.T) / denom) * gamma + beta
...
def batchnorm_backward(dout, cache):
  ...
  temp1 = np.sum((x - sample_mean) * (gamma * dout), axis=0)

  temp2 = ((sample_var + eps) ** -0.5) * temp1
  temp3 = (-0.5) * ((sample_var + eps) ** -1.5) * temp1

  temp4 = np.ones_like(x) / N * temp3
  temp5 = 2 * (x - sample_mean) * temp4

  temp6 = -1 * np.sum(temp5 + temp2, axis=0)
  dx = (np.ones_like(x) / N * temp6) + temp5 + temp2

  denom = np.sqrt(sample_var.T  + eps)
  x_hat = (x - sample_mean.T) / denom
  ...
  for n in range(N):
      for i in range(H_dash):
        for j in range(W_dash):
          for f in range(F):
            curr_matrix = x_pad[n,:, i*stride: i*stride+HH, j*stride:j*stride+WW]
            curr_filter = w[f] 
            out[n,f,i,j] = np.sum(curr_matrix*curr_filter)
  ...
  for n in range(N):
    for f in range(F):
      for i in range(H_dash):
        for j in range(W_dash):
          curr_matrix = x_pad[n,:, i*stride: i*stride+HH, j*stride:j*stride+WW]
          dw[f] = dw[f] + dout[n,f,i,j]* curr_matrix
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in xrange(H_dash):
    for j in xrange(W_dash):
        x_matrix = x[:,:,i*stride : i*stride+HH, j*stride : j*stride+WW]
        out[:,:,i,j] = np.max(x_matrix, axis=(2,3))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in xrange(H_dash):
     for j in xrange(W_dash):
        x_matrix = x[:,:,i*stride : i*stride+HH, j*stride : j*stride+WW]
        x_matrix2 = np.max(x_matrix,axis=(2,3))
        temp = (x_matrix == (x_matrix2)[:,:,None,None])
        dx[:,:,i*stride : i*stride+HH, j*stride : j*stride+WW] += temp * (dout[:,:,i,j])[:,:,None,None]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
